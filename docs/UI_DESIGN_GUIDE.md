...
## 0.5 個性の翻訳（今回の前提）
このサイトは以下の嗜好/属性を軸にUIの“らしさ”を作る：
- 真言密教が好き → 幾何学・規律・余白・朱/墨の緊張感（**抽象表現で敬意を持って**）
- お笑いが好き → コピーやラベルに小さなユーモア。UIは崩さず、**言葉で“クスッ”**
- 坂崎千春などのイラストが好き → 太めアウトライン、素朴な動物モチーフ、色数少なめ
- 動物園が好き → 動物のシルエット/足跡/チケット風UI
- 読書が好き → 目次/しおり/貸出票風モチーフ
- 赤いトップスとショーツと靴下しか着ない → **アクセントは朱/赤で統一**（1色ルールの主役）
...
### P4. 個性は“構造”ではなく“ディテール”で出す
- レイアウトは素直に、**モチーフは小さく反復**（ラベル、区切り線、スタンプ、セクション見出し）
- ユーモアはコピーで出す（視覚ギミックは控えめ）
- 動物/本/幾何学は**線画 + 1色**で統一
...
- CTAボタン（メール）
- `InterestChips`（6個まで、嗜好/属性のタグ。例：Shingon / Comedy / Illustration / Zoo / Books / Red）
...
- 文字は小さめ、**太枠 + 影なし**（軽く）
- 1行に収まらなければ自然にwrap
- 色は基本fg、**accentは1〜2個だけ**に限定

### 6.12 Icon/Illustration（線画モチーフ）
- SVG推奨（inline/外部どちらでもOK）
- 太め線（2px〜3px）、角はスクエア寄り
- 色数は**fg + accentの2色まで**
- 動物・本・幾何学は“記号化”して使う（リアル描写はしない）
...
- [ ] accentは朱/赤に統一され、**多用されていない**
- [ ] 動物/本/幾何学の線画が“1ルール”で揃っている
